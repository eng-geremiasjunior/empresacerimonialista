...
export default async function AdminSuportePage({
  ...
  const mensagens = aberta ? await getConversaSuporte(aberta) : [];
        ...
        <p className="mt-1 text-sm text-stone-500">
          {conversas.length === 0
            ...
            : `${conversas.length} ${conversas.length === 1 ? "conversa" : "conversas"}${
              ...
              }. A resposta aparece na caixinha da pessoa, dentro do sistema.`}
        </p>
      </div>
              ...
              return (
                <li key={c.userId} className="border-b border-stone-100 last:border-b-0">
                  ...
                  >
                    ...
                    <p className="truncate text-xs text-stone-500">{c.empresa}</p>
          ...
          {atual && (
            <section className="flex min-h-[420px] flex-col overflow-hidden rounded-xl border border-stone-200 bg-white">
              ...
              <div className="flex-1 space-y-2 overflow-y-auto bg-stone-50 px-5 py-4">
                {mensagens.map((m) => (
                  <div key={m.id} className={`flex ${m.autor === "eorganizei" ? "justify-end" : "justify-start"}`}>
                    <div
                    ...
                    >
                      ...
                      <p className={`mt-1 text-[10px] ${m.autor === "eorganizei" ? "text-white/60" : "text-stone-400"}`}>
                        {quando(m.em)}
                        {/* em que tela ela estava — metade das dúvidas se
                            responde sabendo isso */}
                        {m.autor === "cliente" && m.pagina ? ` · estava em ${m.pagina}` : ""}
                      </p>
                    </div>
                  </div>
                ))}
