-- ============================================================
-- Vela — Migração 049: evento gerado de orçamento nasce completo
--
-- A 044 já criava evento + cliente + checklist + fases em transação única.
-- Faltavam duas coisas:
--   * roteiro_items (timeline sugerida do template do wizard)
--   * transactions de receita a partir dos itens do orçamento aprovado
--
-- DECISÕES:
--  * O roteiro vem por parâmetro (p_roteiro), como tasks/phases: o template
--    é regra de negócio do app (lib/event-templates.ts) e não deve ser
--    duplicada em SQL.
--  * As transações NÃO vêm por parâmetro: os itens já estão no banco
--    (orcamento_itens). Passá-los pelo cliente permitiria a um chamador
--    anônimo inventar valores — o hash é credencial de leitura, não de
--    arbitrar o financeiro.
--  * due_date das receitas = data do evento, paid = false. O orçamento
--    guarda condições de pagamento da EMPRESA (entrada/parcelas), não um
--    plano por item; parcelar aqui seria inventar regra. A cerimonialista
--    ajusta no Financeiro.
--  * DROP da assinatura antiga de 4 args: `create or replace` com um
--    parâmetro a mais cria uma função NOVA, deixando as duas no schema.
--
-- Execute no SQL Editor do Supabase (depois da 048).
-- ============================================================

begin;

drop function if exists public.criar_evento_do_orcamento(text, jsonb, jsonb, date);
...
begin
  ...
  -- NOVO: timeline sugerida do template (sem horários — a cerimonialista
  -- define no Cronograma; o template só dá a espinha dorsal do dia).
  ...
  -- NOVO: o que foi vendido no orçamento vira receita prevista do evento.
  -- Lido do banco, não do parâmetro: valor acordado não se recebe de fora.
